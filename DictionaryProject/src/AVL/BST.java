...
public class BST<T extends Comparable<? super T>> {
    ...
    public void recInsert(String el) {
        root = recInsert(root,el);
    }
    
    protected BSTNode<T> recInsert(BSTNode<T> p, String el) {
        if (p == null)
             p = new BSTNode<T>(el);
    ...
    public boolean isInTree(String el) {
        return search(el) != null;
    }
    
    protected String search(String el) {
        BSTNode<T> p = root;
        while (p != null)
            if (el.equals(p.el))
                 return p.el;
    ...
    public void balance(String[] data, int first, int last) {
        if (first <= last) {
